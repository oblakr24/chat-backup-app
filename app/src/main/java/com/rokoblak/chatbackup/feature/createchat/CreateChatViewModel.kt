...
class CreateChatViewModel @Inject constructor(
    ...
    private fun CreateChatPresenter(
        contactsFlow: Flow<OperationResult<List<Contact>, ContactsLoadError>?>,
    ): CreateChatUIState {
        return when (val loadResult = contactsFlow.collectAsState(null).value) {
            null -> return CreateChatUIState(Content.Loading)
            is OperationResult.Error -> return CreateChatUIState(Content.Empty("Error loading: $loadResult"))
